...
writeAlternativeComparisonTable <- function(data,variables,file,analysisTypes,labels){
	...
	for(i in 1:length(variables)){
		tableRow=data.frame(rownames=labels[[variables[i]]])
		tableRow<-cbind(tableRow,mean(descriptiveData[,variables[i]],na.rm=TRUE))
		tableRow<-cbind(tableRow,t(quantile(descriptiveData[,variables[i]],probs=c(0.05,0.5,0.95),na.rm=TRUE)))

		for(j in 2:length(analysisTypes)){

			# Calculate mean difference
			
			tableRow<-cbind(tableRow,mean(comparisonData[,paste(varname,1,sep=".")]-comparisonData[,paste(varname,i,sep=".")]))
