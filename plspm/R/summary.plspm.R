...
function(object, ...)
{
    ## x$model <- list(IDM, blocks, scheme, modes, scaled, boot.val, plsr, obs, br, tol, iter, n.iter, outer)
    ## x.fit$model <- list(IDM, blocks, scheme, modes, scaled, obs, tol, iter, n.iter, outer)
    y <- object
    IDM <- y$model$IDM
    ...
    inputs <- data.frame(Block=rownames(IDM), Type=exo.endo, 
                  NMVs=y$model$blocks, Mode=Mode)
    rownames(inputs) <- 1:length(exo.endo)  
    if (length(y$model)==10) {
        res <- list(inputs=inputs, outer.mod=y$outer.mod, inner.mod=y$inner.mod, xxx=y$model)
    } else {
        ...
        if (y$model$boot.val)  # if boot.val=TRUE
            res <- list(inputs=inputs, unidim=y$unidim, outer.mod=y$outer.mod, 
                    outer.cor=y$outer.cor, inner.mod=y$inner.mod, latent.cor=lat.cor,
