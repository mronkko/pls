...
elastic-mapreduce --create --stream --input s3n://$INPUT/input.txt --output s3n://$OUTPUT/  --mapper s3n://$INPUT/map.R --reducer s3n://$INPUT/reduce.R  --cache-archive s3n://$INPUT/cache.zip#include --log-uri s3://$LOGS/ --instance-type c1.medium --instance-count 2  --bootstrap-action s3n://$INPUT/bootstrap.sh  --enable-debugging --ssh --bootstrap-action s3://elasticmapreduce/bootstrap-actions/configure-hadoop --args "-m,mapred.reduce.tasks=`cat  input.txt | wc -l | sed 's/^[^0-9]*//'`"
