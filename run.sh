...
elastic-mapreduce --create --stream --alive --input s3://$INPUT/input.txt --output s3://$OUTPUT/  --mapper s3://$INPUT/map.R --reducer s3://$INPUT/reduce.R --bootstrap-action s3://$INPUT/bootstrap.sh --cache-archive s3://$INPUT/cache.zip#include --log-uri s3://$LOGS/ --instance-type c1.medium  --instance-count 5
